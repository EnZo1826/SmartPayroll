...
let layoutConfig = {
  ...
  // Spacing controls (mm) — defaults match current hard-coded values
  spacingAfterHeader:5,    // gap between header and employee box
  spacingEmployeeBox:38,   // height of employee info box
  spacingAfterEmpBox:5,    // gap after employee box to earnings
  spacingSectionGap:12,    // gap between earnings/deductions and net pay sections
  spacingRowHeight:5,      // line height for each earning/deduction row
  spacingAfterNetPay:8,    // gap after net pay block
};
...
function buildPayslipPDF(empData, lc, run) {
  ...
  line(startY,margin,w-margin,pc); startY+=(lc.spacingAfterHeader??5);

  // ---- EMPLOYEE BOX ----
  const _empBoxH = (lc.spacingEmployeeBox??38);
  doc.setFillColor(248,250,252); doc.rect(margin,startY-2,w-margin*2,_empBoxH-4,'F');
  doc.setDrawColor(230,235,240); doc.rect(margin,startY-2,w-margin*2,_empBoxH-4,'S');
  doc.setFillColor(...pc); doc.rect(margin,startY-2,1.5,_empBoxH-4,'F');
  ...
  const _ef3y = (_empBoxH-14); // third row offset
  field('Bank Account',bankStr,col1,startY+_ef3y);
  field('Pay Period',period,col2,startY+_ef3y);
  startY+=_empBoxH+(lc.spacingAfterEmpBox??5)-5;
  ...
  if(lc.twoColumns) {
    ...
    let ey=startY+7;
    const _rh=lc.spacingRowHeight??5;
    const earningRowL=(label,amt)=>{ doc.setFontSize(8);doc.setFont(pFont,'normal');doc.setTextColor(50,50,50);doc.text(label,margin,ey);const s=fmtN(amt);const tw=doc.getTextWidth(s);doc.text(s,margin+colW-tw,ey);ey+=_rh; };
    ...
    let dy=startY+7;
    const dedRowL=(label,amt)=>{ doc.setFontSize(8);doc.setFont(pFont,'normal');doc.setTextColor(120,30,30);doc.text(label,dx,dy);const s=fmtN(amt);const tw=doc.getTextWidth(s);doc.text(s,dx+colW-tw,dy);dy+=_rh; };
    ...
    const earningRow=(label,amt)=>{
      ...
      const s=fmtN(amt);const tw=doc.getTextWidth(s);doc.text(s,w-margin-tw,startY);startY+=(lc.spacingRowHeight??5);
    };
    ...
    textR(startY+5,fmtN(c.grossEarnings),9.5,'bold',pc); startY+=(lc.spacingSectionGap??12);
    // Deductions
    ...
    textR(startY+5,fmtN(c.totalDeductions),9.5,'bold',[160,30,30]); startY+=(lc.spacingSectionGap??12);
  }
  ...
  if(lc.netPayStyle==='box') {
    ...
    startY+=(lc.spacingAfterNetPay??8)+10;
    ...
    startY+=(lc.spacingAfterNetPay??8);
    ...
    startY+=(lc.spacingAfterNetPay??8)+4;
  }
...
function printPayslip(month,year,empId) {
  ...
  // Net pay HTML
  let netHtml='';
  if(lc.netPayStyle==='box') netHtml=`<div style="background:${np};color:#fff;padding:10px 16px;display:flex;justify-content:space-between;font-size:14px;font-weight:bold;margin:${((lc.spacingSectionGap??12)*2.835).toFixed(0)}px 0 ${((lc.spacingAfterNetPay??8)*2.835).toFixed(0)}px 0;border-radius:4px"><span>NET PAY</span><span>${fmtN(c.netPay)}</span></div>`;
  else if(lc.netPayStyle==='banner') netHtml=`<div style="border:2px solid ${np};border-radius:4px;margin:${((lc.spacingSectionGap??12)*2.835).toFixed(0)}px 0 ${((lc.spacingAfterNetPay??8)*2.835).toFixed(0)}px 0"><div style="background:${np};color:#fff;text-align:center;padding:3px;font-size:9px;font-weight:bold;letter-spacing:2px">NET PAY – ${period}</div><div style="text-align:center;padding:8px;font-size:20px;font-weight:bold;color:${np}">${fmtN(c.netPay)}</div></div>`;
  else netHtml=`<div style="display:flex;justify-content:space-between;padding:8px 5px;border-top:2px solid ${np};border-bottom:2px solid ${np};margin:${((lc.spacingSectionGap??12)*2.835).toFixed(0)}px 0 ${((lc.spacingAfterNetPay??8)*2.835).toFixed(0)}px 0;font-weight:bold;color:${np};font-size:13px"><span>NET PAY</span><span>${fmtN(c.netPay)}</span></div>`;
  ...
  <style>*{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;color-adjust:exact!important;box-sizing:border-box}body{font-family:${fontFamily};margin:0;padding:0;color:#222;font-size:11px}.row{display:flex;justify-content:space-between;padding:${Math.max(0,(lc.spacingRowHeight??5)*0.7-1).toFixed(1)}px 0;line-height:${(lc.spacingRowHeight??5)*0.6+0.8}em}.emp-box{background:#f8fafc!important;border:1px solid #e2e8f0;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-top:${((lc.spacingAfterHeader??5)*2.835).toFixed(0)}px;margin-bottom:${((lc.spacingAfterEmpBox??5)*2.835).toFixed(0)}px;min-height:${((lc.spacingEmployeeBox??38)*2.835).toFixed(0)}px;border-left:4px solid ${p}!important}.emp-label{color:#888;font-size:9px;text-transform:uppercase}.emp-val{font-weight:bold;font-size:11px}@media print{.no-print{display:none}@page{margin:0}}</style>
  </head><body>
  ...
  <div style="margin-top:${((lc.spacingSectionGap??12)*2.835).toFixed(0)}px;font-weight:bold;color:#991b1b;border-bottom:2px solid #f87171;padding-bottom:2px;margin-bottom:5px;font-size:10px">DEDUCTIONS</div>
...
function lcUpdate() {
  ...
  // Spacing controls
  const _sp = id => { const el=document.getElementById(id); return el ? (parseFloat(el.value)||0) : null; };
  if(_sp('lc-sp-after-header')  !== null) layoutConfig.spacingAfterHeader  = _sp('lc-sp-after-header');
  if(_sp('lc-sp-emp-box')       !== null) layoutConfig.spacingEmployeeBox  = _sp('lc-sp-emp-box');
  if(_sp('lc-sp-after-emp')     !== null) layoutConfig.spacingAfterEmpBox  = _sp('lc-sp-after-emp');
  if(_sp('lc-sp-section-gap')   !== null) layoutConfig.spacingSectionGap   = _sp('lc-sp-section-gap');
  if(_sp('lc-sp-row-height')    !== null) layoutConfig.spacingRowHeight     = _sp('lc-sp-row-height');
  if(_sp('lc-sp-after-net')     !== null) layoutConfig.spacingAfterNetPay  = _sp('lc-sp-after-net');
...
function loadLayoutEditorForm() {
  ...
  // Spacing controls
  const _ssv = (id, val) => { const el=document.getElementById(id); if(el) el.value=val; };
  _ssv('lc-sp-after-header', lc.spacingAfterHeader  ?? 5);
  _ssv('lc-sp-emp-box',      lc.spacingEmployeeBox  ?? 38);
  _ssv('lc-sp-after-emp',    lc.spacingAfterEmpBox  ?? 5);
  _ssv('lc-sp-section-gap',  lc.spacingSectionGap   ?? 12);
  _ssv('lc-sp-row-height',   lc.spacingRowHeight    ?? 5);
  _ssv('lc-sp-after-net',    lc.spacingAfterNetPay  ?? 8);
...
function buildPreviewHTML() {
  ...
  // mm → px for 595px-wide A4 iframe (1mm = 2.835px)
  const mm = v => `${Math.round((v||0) * 2.835)}px`;
  const spAfterHeader = lc.spacingAfterHeader  ?? 5;
  const spEmpBox      = lc.spacingEmployeeBox  ?? 38;
  const spAfterEmp    = lc.spacingAfterEmpBox  ?? 5;
  const spRowHeight   = lc.spacingRowHeight    ?? 5;
  const spSectionGap  = lc.spacingSectionGap   ?? 12;
  const spAfterNet    = lc.spacingAfterNetPay  ?? 8;
  ...
  const empBoxHtml = `<div style="background:#f8fafc;border:1px solid #e2e8f0;padding:10px 14px;display:grid;grid-template-columns:1fr 1fr;gap:5px 12px;margin-top:${mm(spAfterHeader)};margin-bottom:${mm(spAfterEmp)};min-height:${mm(spEmpBox)};border-left:4px solid ${p}">
  ...
  if(lc.twoColumns) {
    sectionsHtml = `<table style="width:100%;border-collapse:collapse"><tr>
      <td style="width:50%;vertical-align:top;padding-right:8px">
        ...
        ${earnRows.replace(/class="row"/g,`style="display:flex;justify-content:space-between;padding:${Math.round(spRowHeight*2.835/2-1)}px 0;font-size:9px"`).replace(/class="row total"/g,`style="display:flex;justify-content:space-between;padding:3px 5px;font-size:9px;margin-top:2px"`)}
      </td>
      <td style="width:50%;vertical-align:top;padding-left:8px;border-left:1px solid #e2e8f0">
        ...
        ${dedRows.replace(/class="row"/g,`style="display:flex;justify-content:space-between;padding:${Math.round(spRowHeight*2.835/2-1)}px 0;font-size:9px"`).replace(/class="row total"/g,`style="display:flex;justify-content:space-between;padding:3px 5px;font-size:9px;margin-top:2px"`)}
      </td>
    </tr></table>`;
  } else {
    sectionsHtml = `
      ...
      ${earnRows.replace(/class="row"/g,`style="display:flex;justify-content:space-between;padding:${Math.round(spRowHeight*2.835/2-1)}px 0;font-size:9px"`).replace(/class="row total"/g,`style="display:flex;justify-content:space-between;padding:3px 5px;font-size:9px;margin-top:2px"`)}
      <div style="margin-top:${mm(spSectionGap)};font-size:9px;font-weight:bold;color:#991b1b;border-bottom:2px solid #f87171;padding-bottom:3px;margin-bottom:5px;text-transform:uppercase">DEDUCTIONS</div>
      ${dedRows.replace(/class="row"/g,`style="display:flex;justify-content:space-between;padding:${Math.round(spRowHeight*2.835/2-1)}px 0;font-size:9px"`).replace(/class="row total"/g,`style="display:flex;justify-content:space-between;padding:3px 5px;font-size:9px;margin-top:2px"`)}`;
  }
  ...
  let netPayHtml = '';
  if(lc.netPayStyle==='box') {
    netPayHtml = `<div style="background:${np};color:#fff;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;border-radius:4px;margin:${mm(spSectionGap)} 0 ${mm(spAfterNet)} 0">
      <span style="font-size:14px;font-weight:bold">NET PAY</span><span style="font-size:18px;font-weight:bold">N$31,927.67</span>
    </div>`;
  } else if(lc.netPayStyle==='banner') {
    netPayHtml = `<div style="margin:${mm(spSectionGap)} 0 ${mm(spAfterNet)} 0;border:2px solid ${np};border-radius:4px;overflow:hidden">
    ...
    netPayHtml = `<div style="display:flex;justify-content:space-between;padding:8px 5px;border-top:2px solid ${np};border-bottom:2px solid ${np};margin:${mm(spSectionGap)} 0 ${mm(spAfterNet)} 0">
